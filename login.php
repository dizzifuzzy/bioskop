    ...
    <div class="container">
      ...
      <form method="post" action="" class="form-horizontal">
        ...
        if($_POST){
            ...
            $userpass   = $_POST['pswd'];
            $cek_login = $conn->query("SELECT username_customer,email_customer,password_customer,id_customer FROM customer WHERE username_customer = '$username' OR email_customer = '$username'");
            $f = $cek_login->fetch_assoc();
            $user_password = $f['password_customer'];
            if($cek_login->num_rows==1){
              if(password_verify($userpass,$user_password)) {
                session_start();
                $_SESSION['username'] = $username;
                $_SESSION['password'] = $userpass;
                $_SESSION['id_customer'] = $f['id_customer'];
                header("location:index.php");
            } else {
