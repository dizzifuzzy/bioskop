...
?>
      ...
      <form method="post" action="">
                <?php
        if($_POST){
          $saldo = $conn->query("SELECT saldo_customer FROM customer WHERE username_customer = '$username' OR email_customer = '$username'");
            while($saldox = $saldo->fetch_assoc()) {
                $saldosaya = $saldox['saldo_customer'];
            }
          $chk            = "";
          $select= $_POST['seat'];
          $hitung = count($select);
          ...
          if ($saldosaya < $total_harga) {
            echo '<div class="alert alert-danger">Maaf Saldo Anda Kurang!<p>Saldo anda : '.$saldoo['saldo_customer'].'</div>';
          }elseif(!$select){
            ...
            $a = $conn->query("INSERT INTO tiket (id,tanggal,tanggal_tonton,id_customer,film_id,judul,id_studio,nama_studio,seat,jam_tonton,jumlah_orang,harga,total_harga,kodebooking,status) VALUES (NULL,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,'$id_customer','$id','$inifilm','$idstudio','$namastudio','$chk','$jamsaya','$hitung','$hargastudio','$total_harga','$kodebooking','Ready To Watch')");
            if ($a) {
              $update = $conn->query("UPDATE customer SET saldo_customer = saldo_customer - $total_harga WHERE username_customer = '$username' OR email_customer = '$username'");
              if ($update) {
              ...
              <div class="col-sm-12">
                <label for="nama" class="col-sm-12 control-label">
                  ...
                  if($seat->num_rows<1){
                    ...
                    $sass = $conn->query("SELECT * FROM tiket where film_id='$id'");
